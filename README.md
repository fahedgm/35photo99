...
**Current version: v1.11.** The last known-good reference build is v1.7
"35photoUltimate" — see `CHANGELOG.md` for what each contains.
...
Manual mode is the **default** on a fresh install, and the mode is
remembered across launches. ISO defaults to the sensor's lowest value.

**Fully WYSIWYG.** The preview always runs at the pinned ISO and the exact
shutter that will be used, so the live image is what gets saved — including
motion blur from a slow shutter. There is no case where preview and capture
use different exposure.

**Auto shutter via the app's own metering.** Camera2 has no ISO-priority
mode: `AE_MODE_ON` picks both ISO and shutter, `AE_MODE_OFF` picks neither,
and AE will not report a scene reading while it's off. So with ISO pinned
and AE off, the app meters for itself — grabbing a 32x24 sample off the
TextureView already displaying the preview every 400ms, measuring mean
luma, and correcting the shutter toward a mid-grey target.

Three details that matter in that loop:

- **Gamma.** Preview luma is gamma-encoded, so doubling exposure time does
  not double the measured value. The perceptual error ratio is raised to
  2.2 to convert it back to a linear exposure ratio, which is what makes
  this converge in one or two passes instead of creeping toward correct.
- **Deadband.** Changes under 8% are ignored, so a static scene settles
  instead of hunting back and forth indefinitely.
- **Step clamp.** A single pass can change exposure by at most 4x, so a
  sudden bright flash can't lurch the exposure wildly.

**Reading brightness off the TextureView rather than a camera stream** is
deliberate: a dedicated analysis stream would mean four concurrent streams
alongside preview + JPEG + RAW, and some devices won't permit that. The
TextureView is already there, and measuring it measures exactly what will
be captured.

**Taking manual control.** Dragging the shutter slider stops the metering
loop; the value stays where you put it. Tap the shutter label (or switch
Auto -> Manual) to hand it back to auto-metering. Changing ISO does *not*
stop auto-metering — the loop simply sees the new brightness on its next
pass and re-converges. The shutter label and status bar both show which
state is active.

ISO and shutter both require the device's `MANUAL_SENSOR` capability; on a
device without it, Manual falls back to normal auto-exposure and the
sliders hide themselves.
